...
export function getHeaders(range: DefaultTheme.Config["outline"]): Array<MenuItem> {
    const headers = [
...
function buildTree(data: Array<MenuItem>, min: number, max: number): Array<MenuItem> {
    resolvedHeaders.length = 0;

    const result: Array<MenuItem> = [];
    const stack: Array<MenuItem | { level: number; shouldIgnore: true }> = [];
